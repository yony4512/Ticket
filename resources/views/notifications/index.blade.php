            ...
            @forelse($notifications as $notification)
                <div class="flex items-start space-x-4 px-6 py-4 {{ !$notification->read_at ? 'bg-blue-50' : '' }}">
                    <div class="w-10 h-10 rounded-full flex items-center justify-center mt-1"
                         style="background: {{ $notification->type === 'ticket_purchased' ? 'linear-gradient(135deg,#3b82f6,#6366f1)' : ($notification->type === 'event_created' ? 'linear-gradient(135deg,#22c55e,#16a34a)' : ($notification->type === 'message_received' ? 'linear-gradient(135deg,#a21caf,#6366f1)' : 'linear-gradient(135deg,#f59e42,#fbbf24)')) }}; color: white;">
                        <i class="fas {{
                            $notification->type === 'ticket_purchased' ? 'fa-ticket-alt' :
                            ($notification->type === 'event_created' ? 'fa-calendar' :
                            ($notification->type === 'message_received' ? 'fa-envelope' : 'fa-bell'))
                        }} text-lg"></i>
                    </div>
